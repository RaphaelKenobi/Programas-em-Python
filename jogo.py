...
def ModoDeJogo():
    ...
    print("Você selecionou o modo: {}\n".format(
        modosDeJogo[modoDeJogoSelecionado - 1]))
...
def partida():
    ...
    while totalDePecas > 0:
        ...
        if turnoDoComputador:
            pecasRetiradas = computador_escolhe_jogada(
                totalDePecas, limitePecasPorJogada)
            print("O computador tirou {} peca(s).".format(pecasRetiradas))
        else:
            pecasRetiradas = usuario_escolhe_jogada(
                totalDePecas, limitePecasPorJogada)
...
def campeonato():
    ...
    # exibe o placa final
    print("Placar: Você {} X {} Computador".format(
        pontosDoUsuario, pontosDoComputador))
