...
# loop de valores
    while True:
        c_obtidas = int(input())
        soma += 1
        abacaxi(orcamento, valor_booster, q_desejada, soma, x)
        if (c_obtidas == 5 or c_obtidas == 3):
            x += 1
        break


def abacaxi(orcamento, valor_booster, q_desejada, soma, x):
    ...
    print(f"QUANTIDADE OBTIDA DE CARTAS DA COR VERDE OU DA COR PRETA: {x}")
    if x == q_desejada:
